...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if mel_spectrogram.shape[1] < self.max_length:
            pad_width = self.max_length - mel_spectrogram.shape[1]
            mel_spectrogram = np.pad(mel_spectrogram, ((0, 0), (0, pad_width)), mode='constant')
        else:
            mel_spectrogram = mel_spectrogram[:, :self.max_length]

        # 转换为张量，确保它是 3D 张量 [channels, height, width]
        mel_spectrogram_tensor = torch.tensor(mel_spectrogram).unsqueeze(0).float()  # 添加通道维度，成为 [1, n_mels, time]
...
if __name__ == '__main__':
    ...
    # 创建训练集和测试集的数据集对象
    train_dataset = AudioDataset(train_paths, train_labels, n_mels=64, max_length=431)
